...
function SupplyBagLabels() {
  return (
    <span className="banner-bag-labels">
      {Array.from({ length: 8 }, (_, index) => (
        <span className={`banner-bag-label banner-bag-label-${index}`} key={index}>
          <strong>NIMIR</strong><span>PLAM BRIGHT</span>
        </span>
      ))}
    </span>
  );
...
const banners: Banner[] = [
  ...
  },
  {
    ...
    artworkAlt: `IMOKO-branded delivery truck and shipping container at Imoko warehouse, ${warehouseAddress} White NIMIR PLAM BRIGHT bags remain stacked on pallets, with a loading sequence moving a pallet from the warehouse into the rear of the truck.`,
  },
  {
                    ...
                    {banner.items.map(item => {
                      ...
                      <span className={`banner-piece banner-piece-${item}`} key={item}>
                        <span className="banner-piece-motion">
                          ...
                          {banner.theme === 'supply' && item === 'warehouse' && (
                          ...
                          {banner.theme === 'supply' && item === 'pallet' && <SupplyBagLabels />}
                        </span>
                      </span>
                      );
                    })}
                    {banner.theme === 'supply' && (
                      <span className="banner-piece banner-piece-pallet banner-loading-pallet">
                        <span className="banner-piece-motion">
                          <span className="banner-piece-image" style={{
                            backgroundImage: `url(/images/${artworkLayers.pallet.file})`,
